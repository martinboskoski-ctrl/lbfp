...
const GateTimeline = ({ gates, currentGate }) => {
  ...
  return (
    <div className="card p-5">
      <h2 className="text-sm font-semibold text-gray-700 mb-4">{t('gate.workflowProgress')}</h2>
      <div className="relative">
        <div className="absolute left-[9px] top-5 bottom-5 w-0.5 bg-gray-200" />
        <div className="space-y-4">
          {(gates || []).map((gate, i) => {
            const fullLabel = gateLabels[i] || '';
            const [gateName, gateDesc] = fullLabel.split(' — ');
            ...
            return (
              <div key={i} className="relative flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0 pb-1">
                  ...
                  <p className="text-xs text-gray-400">{gateDesc}</p>
